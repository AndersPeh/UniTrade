...
public class User {
  ...
  // Tells JPA that this must be unique so Hibernate can use it to identify a User in queries and
  // caching.
  ...
  // When user is deleted, the role should remain so dont put cascade = CascadeType.ALL,
  // orphanRemoval = true.
  // fetch = FetchType.EAGER means when a User entity is loaded, its associated Role entities are
  // loaded immediately,
  ...
  // The owning side has to configure the join table (user_roles), define 2 primary/ foreign keys
  // (user_id, role_id),
  // joinColumns refers to the id in owning side (User), inverseJoinColumns refers to the id in
  // inverse side (Role).
  @JoinTable(
      name = "user_roles",
  ...
  // it can only store user_id and role_id. Need to use Many to Many via explicit entity like
  // CartItem to store more infomation.
